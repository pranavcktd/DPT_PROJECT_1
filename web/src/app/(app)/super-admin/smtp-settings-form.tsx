...
import { smtpSettingsFormSchema, type SmtpSettingsFormInput, type SmtpSettingsFormValues } from "../department/smtp-schema";
import { upsertSuperAdminSmtpSettings, sendSuperAdminTestEmail } from "./smtp-actions";
...
export function SuperAdminSmtpSettingsForm({ smtp }: { smtp: ExistingSmtp }) {
  ...
  async function onSubmit(values: SmtpSettingsFormValues) {
    ...
    const result = await upsertSuperAdminSmtpSettings({ error: null }, formData);
    if (result.error) {
  ...
  async function handleTestEmail() {
    ...
    const result = await sendSuperAdminTestEmail({ error: null });
    setTestState({
      pending: false,
  ...
  return (
    <Card className="max-w-2xl">
      <CardHeader>
        ...
        <CardDescription>
          Used for Super Admin emails (e.g. sending reports or notices out yourself).{" "}
          {smtp.isConfigured ? "Currently configured." : "Not configured yet."}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="Software Company" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
